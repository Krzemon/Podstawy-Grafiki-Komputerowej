...
void hexagon::Draw_Border(sf::RenderTarget& target, sf::RenderStates states, sf::String name) const
{
    ...
    sf::Vector2f position = center - sf::Vector2f(50.0f, 20.0f); // pozycja 
    sf::VertexArray border(sf::LineStrip, 7);
    for (int i = 0; i < 6; ++i) {
        border[i].position = p[i]; // punkty chronione klasy hexagon
    }
    border[6].position = p[0];

    border[0].color = sf::Color::Black;
    border[1].color = sf::Color::Black;
    border[2].color = sf::Color::Black;
    border[3].color = sf::Color::Black;
    border[4].color = sf::Color::Black;
    border[5].color = sf::Color::Black;
    border[6].color = sf::Color::Black;
...
// Napisać aplikację, która na ekranie wyrysuje cztery sześciokąty barw oraz „suwak” pozwalający ustawić na przykład parametr L w sześciokącie HSL.Każdy sześciokąt będzie przedstawiał barwy w innej reprezentacji
...
void hexagon_RGB::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
    ...
    sf::ConvexShape hexagonShape;
    hexagonShape.setPointCount(6);
    for (int i = 0; i < 6; ++i)
        hexagonShape.setPoint(i, p[i]);
    hexagonShape.setFillColor(sf::Color::Red);
...
void hexagon_CMY::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
    ...
    for (int i = 0; i < 6; ++i)
        hexagonShape.setPoint(i, p[i]);
    hexagonShape.setFillColor(sf::Color::Cyan);
...
{    
    ...
    color.r = color.g = color.b = static_cast<int>(L * 255);
    hexagonShape.setFillColor(color);
...
void hexagon_HSB::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
    ...
    for (int i = 0; i < 6; ++i)
        hexagonShape.setPoint(i, p[i]);
    hexagonShape.setFillColor(sf::Color::Magenta);
